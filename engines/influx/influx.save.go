...
func getSaveParams(ctx *context.Context) (measurement string, tags map[string]string, fields map[string]interface{}, err error) {
	...
	measurement, err = ctx.Request.Form.Get("measurement")
	...
	tagStr, err := ctx.Request.Form.Get("tags")
	if err != nil {
		err = errors.New("form中未包含tags标签")
		return
	}
	fieldStr, err := ctx.Request.Form.Get("fields")
	if err != nil {
