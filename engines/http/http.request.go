...
//Request 请求转跳
func Request() component.WebServiceFunc {
...
func getData(u url.Values, data conf.Conf, trs *transform.Transform) (ua url.Values, rawStr string, err error) {
	...
	if !hasEncrypt {
		err := fmt.Errorf("%s加密方式不支持，只支持:%v", encrypt, encrypts)
		return u, "", err
	}
	kc := data.String("_c", "=")
