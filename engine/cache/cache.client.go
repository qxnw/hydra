...
func (s *cacheProxy) getMemcacheClient(ctx *context.Context) (*memcache.MemcacheClient, error) {
	...
	if !ok {
		return nil, fmt.Errorf("engine:cache.args配置错误，不是map[string]string类型:%v", ctx.Input.Args)
	}
	cacheName, ok := args["cache"]
	if cacheName == "" || !ok {
		return nil, fmt.Errorf("engine:cache.args配置错误，缺少cache参数:%v", ctx.Input.Args)
	}
	...
	_, client, err := s.dbs.SetIfAbsentCb(content, func(i ...interface{}) (interface{}, error) {
		...
		if err != nil {
			return nil, fmt.Errorf("engine:cache.args配置错误无法解析:%s(err:%v)", content, err)
		}
		server := cnf.Strings("server")
		if len(server) == 0 {
			return nil, fmt.Errorf("engine:cache配置错误:server 和 dataBase不能为空（server:%v）", server)
		}
...
func (s *cacheProxy) getVarParam(ctx *context.Context, name string) (string, error) {
	...
	if funcVar == nil {
		return "", errors.New("engine:cache.未找到__func_var_get_")
	}
	if f, ok := funcVar.(func(c string, n string) (string, error)); ok {
		return f("cache", name)
	}
	return "", errors.New("engine:cache.未找到__func_var_get_传入类型错误")
}
