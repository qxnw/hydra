...
func (s *collectProxy) responseCollect(tp string) func(ctx *context.Context) (r string, st int, err error) {

	return func(ctx *context.Context) (r string, st int, err error) {
		title := ctx.GetArgValue("title", "请求响应码")
		msg := ctx.GetArgValue("msg", "@url请求响应码:@code在@span内出现:@current次")
