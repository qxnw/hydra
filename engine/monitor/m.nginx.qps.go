...
func (s *monitorProxy) nginxQPSCollect(name string, mode string, service string, ctx *context.Context) (response *context.StandardResponse, err error) {
	response = context.GetStandardResponse()
	ip := net.GetLocalIPAddress(ctx.Input.GetArgsValue("mask", ""))
	c, t, err := s.getNginxQPSCount()
	if err != nil {
		return
	}
...
func (s *monitorProxy) getNginxQPSCount() (m int, tm string, err error) {
