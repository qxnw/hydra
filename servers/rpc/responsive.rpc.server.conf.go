...
func (w *RpcResponsiveServer) NeedRestart(cnf conf.IServerConf) (bool, error) {
	...
	if comparer.IsValueChanged("status", "address", "engines", "host") {
		return true, nil
	}
...
func (w *RpcResponsiveServer) SetConf(restart bool, conf conf.IServerConf) (err error) {
	...
	servers.TraceIf(ok, w.Infof, w.Warnf, conf.GetServerName(), getEnableName(ok), "jwt设置")

	//设置请求头
	if ok, err = SetHeaders(w.server, conf); err != nil {
		return err
	}
	servers.TraceIf(ok, w.Infof, w.Warnf, conf.GetServerName(), getEnableName(ok), "header设置")
	...
	servers.TraceIf(ok, w.Infof, w.Warnf, conf.GetServerName(), getEnableName(ok), "host设置")
	return nil
}
