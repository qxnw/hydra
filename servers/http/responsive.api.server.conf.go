...
func (w *ApiResponsiveServer) SetConf(conf *responsive.ResponsiveConf) (err error) {
	...
	w.Infof("%s:%s静态文件", conf.GetFullName(), getEnableName(ok))
	...
	w.Infof("%s:%sheader设置", conf.GetFullName(), getEnableName(ok))
	...
	w.Infof("%s:%sajax请求限制设置", conf.GetFullName(), getEnableName(ok))
