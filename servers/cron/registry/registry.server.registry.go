...
func (s *RegistryServer) watchMasterChange(root, path string) error {
	cldrs, _, err := s.engine.GetRegistry().GetChildren(root)
	if err != nil {
		return err
	}
	s.master = s.isMaster(path, cldrs)
	s.Debugf("%s是:%s", s.serverConf.GetFullName(), types.DecodeString(s.master, true, "master server", "slave server"))
	...
	go func() {
		for {
			...
			case cldWatcher := <-children:
				...
				if master != s.master {
					s.Debugf("%s是:%s", s.serverConf.GetFullName(), types.DecodeString(master, true, "master server", "slave server"))
...
func (s *RegistryServer) isMaster(path string, cldrs []string) bool {
	...
	sort.Strings(ncldrs)
	s.shardingCount = types.ToInt(s.serverConf.Get("sharding"), 0)
...
func (s *RegistryServer) notifyConsumer(v bool) error {
	if v {
