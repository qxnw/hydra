...
func NewWatchConf(path string, registry registry.Registry, updater chan []*ServiceUpdater, timeSpan time.Duration) *watchConf {
...
func (w *watchConf) NotifyConfDel() {
	...
	w.cacheAddress.RemoveIterCb(func(key string, value interface{}) bool {
		updater := &ServiceUpdater{Value: key, Op: registry.DEL}
		w.notifyConfChan <- []*ServiceUpdater{updater}
		return true
	})
	w.exists = false
}
...
func (w *watchConf) checkChildrenChange(children []string) {
	...
	for _, v := range children { //检查当前配置地址未缓存
		if _, ok := w.cacheAddress.Get(v); !ok {
			if ok, _, _ := w.cacheAddress.SetIfAbsentCb(v, func(input ...interface{}) (interface{}, error) {
				...
				updaters = append(updaters, &ServiceUpdater{Op: registry.ADD, Value: v})
			}
		}
	...
	w.cacheAddress.RemoveIterCb(func(key string, value interface{}) bool {
		...
		if !exists {
			updaters = append(updaters, &ServiceUpdater{Op: registry.DEL, Value: key})
			return true
		}
		return false
	})
