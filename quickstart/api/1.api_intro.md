...
### 2. 服务器主配置
服务器主配置信息，包含服务器启动的必须参数，路径:`/[platName]/[servers]/[subsystemName]/api/conf/[tagName]`
```json
{
    "address":":8080",
    "status":"start",
...
|status|必须|服务器状态，可选值为: start(启动)，stop(停止)|
...
|参数名|必须|说明|
|:------|:-------:|:------|
|prefix|可空|请求路径前的前缀,如:/static/logo.png,则匹配dir目录下的logo.png文件|
|dir|必须|静态文件存放路径|
|showDir|可空|是否显示目录结构, 默认为:false|
|exts|必须|静态文件的扩展名|
