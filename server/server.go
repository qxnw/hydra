...
type IServerAdapter interface {
	Resolve(c context.ContextHandler, conf registry.Conf) (IHydraServer, error)
}
...
func NewServer(adapter string, c context.ContextHandler, conf registry.Conf) (IHydraServer, error) {
	if resolver, ok := serverResolvers[adapter]; ok {
		return resolver.Resolve(c, conf)
	}
