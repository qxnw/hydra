...
func NewWatchConf(path string, registry Registry, updater chan []*service.ServiceUpdater, timeSpan time.Duration) *watchConf {
...
func (w *watchConf) NotifyConfDel() {
	...
	w.cacheAddress.RemoveIterCb(func(key string, value interface{}) bool {
		updater := &service.ServiceUpdater{Value: key, Op: conf.DEL}
		w.notifyConfChan <- []*service.ServiceUpdater{updater}
		return true
	})
	w.exists = false
}
...
func (w *watchConf) checkChildrenChange(children []string) {
	w.mu.Lock()
	defer w.mu.Unlock()
	updaters := make([]*service.ServiceUpdater, 0, 0)
	...
	for _, v := range children { //检查当前配置地址未缓存
		if _, ok := w.cacheAddress.Get(v); !ok {
			if ok, _, _ := w.cacheAddress.SetIfAbsentCb(v, func(input ...interface{}) (interface{}, error) {
				...
				updaters = append(updaters, &service.ServiceUpdater{Op: conf.ADD, Value: v})
			}
		}
	...
	w.cacheAddress.RemoveIterCb(func(key string, value interface{}) bool {
		...
		if !exists {
			updaters = append(updaters, &service.ServiceUpdater{Op: conf.DEL, Value: key})
			return true
		}
		return false
	})
